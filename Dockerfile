...
COPY --from=build weaver/target/release/kafka-copy-rust ./bin/kafka-copy-thread
COPY --from=build weaver/etc/config.toml ./etc/

EXPOSE 8085

ENTRYPOINT ["./bin/kafka-copy-thread", "--config", "./etc/config.toml"]
